...
public class EMTMJ {
	 ...
	 ************************* 
	 */
	public EMTMJ(int paraUsers, int paraItems) throws Exception {
		...
		// String tempReaderfile = "data/douban/movie/newRatings.txt";
		// //testingSmallset.txt
		String tempReaderfile = "data/eachmovie/eachmovie.dat";
		File tempFile = null;
		...
		while ((tempString = tempBufReader.readLine()) != null) {
			...
			tempRating = (int) (Double.parseDouble(tempStrArray[2]) * 10);
			...
			tempUserIndices[tempUserIndex][userDegree[tempUserIndex]] = tempItemIndex;
			tempUserRatings[tempUserIndex][userDegree[tempUserIndex]] = tempRating;
			userTotalRating[tempUserIndex] += tempRating;
	 ...
	 ************************* 
	 */
	public void leaveOneOut(int paraItemIndex, int paraItemPosition, int paraUserIndex, double paraValue) {
		...
		if (itemDegree[paraItemIndex] > 1e-6) {
			itemAveRating[paraItemIndex] = itemTotalRating[paraItemIndex] / itemDegree[paraItemIndex];
		} else {
	...
	public void leaveOneOutRestore() {
		...
		runSteps++;
		itemAveRating[leaveOutItemIndex] = itemTotalRating[leaveOutItemIndex] / itemDegree[leaveOutItemIndex];
		runSteps++;
	...
	/**
	 ************************* 
	 * Prediction, and return the deviation.
	 * 
	 * @author Fan Min
	 ************************* 
	 */
	public double[] computeOneDeviationKNNMAE(int paraItemIndex, int[] paraUserIndexArray, double[] paraRatingArray,
			int paraK, int paraMeasure) throws Exception {

		double[] tempResult = new double[2];

		for (int i = 0; i < paraUserIndexArray.length; i++) {
			// Step 1. Leave the one out.
			leaveOneOut(paraItemIndex, i, paraUserIndexArray[i], paraRatingArray[i]);

			// Step 2. Predict.
			double tempPrediction = 0;

			if (paraMeasure == 0) {
				tempPrediction = predictKNNDistance(paraItemIndex, i, paraUserIndexArray, paraRatingArray, paraK,
						paraMeasure);
			} else {
				tempPrediction = predictKNN(paraItemIndex, i, paraUserIndexArray, paraRatingArray, paraK, paraMeasure);

			} // of if

			// Of if
			runSteps++;
			tempResult[0] += Math.abs(tempPrediction - leaveOutValue) / 10;
			runSteps++;
			tempResult[1] += (tempPrediction - leaveOutValue) * (tempPrediction - leaveOutValue) / 100;
			runSteps++;
			// Step 4. Restore.
			leaveOneOutRestore();
			runSteps++;
	...
	/**
	 ************************* 
	 * Initialize space, suit Cosine, Jaccard etc.
	 * 
	 * @param paraNeighborsIndice
	 * @param paraNeighborsDistance
	 ************************* 
	 */
	public void InitializeSpaceBigToSmall(int[] paraNeighborsIndice, double[] paraNeighborsDistance,
			int[] paraCheckIndice) {
	 ...
	 ************************* 
	 */
	public void InitializeSpaceSmallToBig(int[] paraNeighborsIndice, double[] paraNeighborsDistance,
			int[] paraCheckIndice) {
		paraNeighborsIndice[0] = -1;
	 ...
	 ************************* 
	 */
	public double predictKNNDistance(int paraPredictItemArrayIndex, int paraUserPosition, int[] paraUserIndexArray,
		...
		InitializeSpaceSmallToBig(tempNeighborIndices, tempNeighborDistances, tempcheckIndices); // 锟斤拷小锟斤拷锟斤拷
		...
		for (int i = 0; i < itemNumber; i++) {
			...
			int tempCheck = Arrays.binarySearch(itemRatingIndices[i], paraUserIndexArray[paraUserPosition]);
			...
			if ((tempDistance == 9999) || (tempDistance > tempNeighborDistances[paraK + 1])) {
				continue;
			} // Of if
			...
			// System.out.println("tempNeighborIndices: "
			// + Arrays.toString(tempNeighborIndices));
			// Step 3. Predict.
		double tempPrediction = 0;
		double tempSumSimilarity = 0;
		for (int i = 1; i <= paraK; i++) {
			if (tempNeighborIndices[i] != -1) {
				neighborScoreTotal += (itemRatingInformation[tempNeighborIndices[i]][tempcheckIndices[i]]
						- itemAveRating[tempNeighborIndices[i]]) * tempNeighborDistances[i];
		...
		if (tempSumSimilarity != 0) {
			tempPrediction = itemAveRating[paraPredictItemArrayIndex] + neighborScoreTotal / tempSumSimilarity;
			if (tempPrediction < 2) {
				tempPrediction = 2;
			}
			if (tempPrediction > 10) {
				tempPrediction = 10;
			}
		} else {
			tempPrediction = 6;
	 ...
	 ************************* 
	 */
	private double predictKNN(int paraPredictItemArrayIndex, int paraUserPosition, int[] paraUserIndexArray,
			double[] paraRatingArray, int paraK, int paraMeasure) throws Exception {
		// Step 1. Find the neighbors.
		neighborScoreTotal = 0;
		int[] tempNeighborIndices = new int[paraK + 2];
		...
		InitializeSpaceBigToSmall(tempNeighborIndices, tempNeighborDistances, tempcheckIndices); // 锟接达拷�?
		...
		for (int i = 0; i < itemNumber; i++) {
			...
			// System.out.println("Distance: " + tempDistance);
			// System.out.println(Arrays.toString(tempNeighborIndices));
			// // Less than the smallest
			if ((tempDistance == -9999) || (tempDistance < tempNeighborDistances[paraK + 1])) {
				continue;
			} // Of if
		...
		for (int i = 0; i < itemRatingInformation[paraRow1].length; i++) {
			for (int j = 0; j < itemRatingInformation[paraRow2].length; j++) {
				if (itemRatingIndices[paraRow1][i] == itemRatingIndices[paraRow2][j]) {
					tempCount++;
					tempDistance += Math.abs(itemRatingInformation[paraRow1][i] - itemRatingInformation[paraRow2][j]);
				} // Of if
			} // Of for j
		} // Of for i
	...
	public double Euclidean(int paraRow1, int paraRow2) throws Exception {
		double tempE = 0;
		if (itemRatingIndices[paraRow1].length == 0 || itemRatingIndices[paraRow2].length == 0) {

			return 0.0;
		} // Of if

		if ((itemRatingIndices[paraRow1][0] > itemRatingIndices[paraRow2][itemRatingIndices[paraRow2].length - 1])
				|| (itemRatingIndices[paraRow2][0] > itemRatingIndices[paraRow1][itemRatingIndices[paraRow1].length
						- 1])) {
		...
		int i = 0, j = 0;
		while (i < itemRatingIndices[paraRow1].length && j < itemRatingIndices[paraRow2].length) {
			if (itemRatingIndices[paraRow1][i] < itemRatingIndices[paraRow2][j]
	...
	public double TMJ(int paraRow1, int paraRow2) throws Exception {
		...
		while (i < itemRatingIndices[paraRow1].length && j < itemRatingIndices[paraRow2].length) {
			if (itemRatingIndices[paraRow1][i] < itemRatingIndices[paraRow2][j]
				...
				tempParaRow1 += itemRatingInformation[paraRow1][i] * itemRatingInformation[paraRow1][i];
				tempParaRow2 += itemRatingInformation[paraRow2][j] * itemRatingInformation[paraRow2][j];
		...
		tempJ = tempJ / (itemRatingIndices[paraRow1].length + itemRatingIndices[paraRow2].length - tempJ - 1);
	...
	public double Cosine(int paraRow1, int paraRow2) throws Exception {
		...
		while (i < itemRatingIndices[paraRow1].length && j < itemRatingIndices[paraRow2].length) {
			if (itemRatingIndices[paraRow1][i] < itemRatingIndices[paraRow2][j]
				...
				tempParaRow1 += itemRatingInformation[paraRow1][i] * itemRatingInformation[paraRow1][i];
				tempParaRow2 += itemRatingInformation[paraRow2][j] * itemRatingInformation[paraRow2][j];
				tempC += itemRatingInformation[paraRow1][i] * itemRatingInformation[paraRow2][j];
	...
	public double PIP(int paraRow1, int paraRow2) throws Exception {
		...
		while (i < itemRatingIndices[paraRow1].length && j < itemRatingIndices[paraRow2].length) {
			if (itemRatingIndices[paraRow1][i] < itemRatingIndices[paraRow2][j]
				...
				if ((itemRatingInformation[paraRow1][i] > 6 && itemRatingInformation[paraRow2][j] < 6)
						|| (itemRatingInformation[paraRow1][i] < 6 && itemRatingInformation[paraRow2][j] > 6)) {
					agreeMent = false;
				}
				...
				if (agreeMent) {
					...
					absoluteDiatance = 2
							* Math.abs(itemRatingInformation[paraRow1][i] - itemRatingInformation[paraRow2][j]);
				}
				tempProximity = (17 - absoluteDiatance) * (17 - absoluteDiatance);
				// System.out.println("tempProximity: "+tempProximity);
				double tempImpact = 0;
				if (agreeMent) {
					tempImpact = (Math.abs(itemRatingInformation[paraRow1][i] - 6) + 1)
							* (Math.abs(itemRatingInformation[paraRow2][j] - 6) + 1);
				} else {
					tempImpact = 1 / ((Math.abs(itemRatingInformation[paraRow1][i] - 6) + 1)
							* (Math.abs(itemRatingInformation[paraRow2][j] - 6) + 1));
				}
				// System.out.println("tempImpact: "+tempImpact);
				double tempPopularity = 0;
				if ((itemRatingInformation[paraRow1][i] > userAveRating[itemRatingIndices[paraRow1][i]]
						&& itemRatingInformation[paraRow2][j] > userAveRating[itemRatingIndices[paraRow1][i]])
						|| (itemRatingInformation[paraRow1][i] < userAveRating[itemRatingIndices[paraRow1][i]]
								&& itemRatingInformation[paraRow2][j] < userAveRating[itemRatingIndices[paraRow1][i]])) {
					double tempPP = (itemRatingInformation[paraRow1][i] + itemRatingInformation[paraRow2][j]) / 2
	...
	public double NHSM(int paraRow1, int paraRow2) throws Exception {
		...
		while (i < itemRatingIndices[paraRow1].length && j < itemRatingIndices[paraRow2].length) {
			if (itemRatingIndices[paraRow1][i] < itemRatingIndices[paraRow2][j]
				...
				double Proximity = 0;
				Proximity = 1 - 1 / (1
						+ Math.exp(-Math.abs(itemRatingInformation[paraRow1][i] - itemRatingInformation[paraRow2][j])));
				// System.out.println("test tempProximity");
				// Impact
				double Significance = 0;
				Significance = 1 / (1 + Math.exp(-Math.abs(itemRatingInformation[paraRow1][i] - 0.5)
						* Math.abs(itemRatingInformation[paraRow2][j] - 0.5)));
				// System.out.println("test tempImpact");
				// Popularity
				double Singularity = 0;
				Singularity = 1 - 1 / (1 + Math
						.exp(-Math.abs((itemRatingInformation[paraRow1][i] + itemRatingInformation[paraRow2][j]) / 2
								- userAveRating[itemRatingIndices[paraRow1][i]])));
		...
		double tempStandardDeviationPredition = Math
				.sqrt(tempChangedeviation / (itemRatingInformation[paraRow1].length - 1));
		double tempStandardDeviationParaRow2 = Math
				.sqrt(standarddeviation[paraRow2] / itemRatingInformation[paraRow2].length);
		tempURP = 1 - 1 / (1 + Math.exp(-Math.abs(itemAveRating[paraRow1] - itemAveRating[paraRow2])
	...
	public double BC(int paraRow1, int paraRow2) throws Exception {
		...
		for (int j = 0; j < itemRatingInformation[paraRow1].length; j++) {
			...
			everyItemClassfication[0][(int) (itemRatingInformation[paraRow1][j] / 2 - 1)]++;
			everyItemClassfication[0][1]++;
		} // of for if
		...
		for (int j = 0; j < itemRatingInformation[paraRow2].length; j++) {
			...
			everyItemClassfication[1][(int) (itemRatingInformation[paraRow2][j] / 2 - 1)]++;
			everyItemClassfication[1][1]++;
		} // of for j

		double BSum = 0;

		for (int x = 0; x < 5; x++) {
			BSum += Math.sqrt((everyItemClassfication[0][x] / everyItemClassfication[0][1])
					* (everyItemClassfication[1][x] / everyItemClassfication[1][1]));
		} // of if X
	...
	public double pearson(int paraRow1, int paraRow2) throws Exception {
		...
		while (i < itemRatingIndices[paraRow1].length && j < itemRatingIndices[paraRow2].length) {
			if (itemRatingIndices[paraRow1][i] < itemRatingIndices[paraRow2][j]
				...
				tempFirstUserRating = itemRatingInformation[paraRow1][i] - itemAveRating[paraRow1];
				tempSecondUserRating = itemRatingInformation[paraRow2][j] - itemAveRating[paraRow2];
	...
	public double CPCC(int paraRow1, int paraRow2) throws Exception {
		...
		while (i < itemRatingIndices[paraRow1].length && j < itemRatingIndices[paraRow2].length) {
			if (itemRatingIndices[paraRow1][i] < itemRatingIndices[paraRow2][j]
				...
				tempFirstUserRating = itemRatingInformation[paraRow1][i] - 0.5;
				tempSecondUserRating = itemRatingInformation[paraRow2][j] - 0.5;
				tempXYSum += tempFirstUserRating * tempSecondUserRating;
				tempXSum += tempFirstUserRating * tempFirstUserRating;
				tempYSum += tempSecondUserRating * tempSecondUserRating;
				i++;
	...
	public double[] computeMAE(int paraK, int paraMeasure) throws Exception {
		...
		for (int i = 0; i < itemRatingInformation.length; i++) {
			...
			double[] tempResult = computeOneDeviationKNNMAE(i, itemRatingIndices[i], itemRatingInformation[i], paraK,
					paraMeasure);
			runSteps++;
		...
		runSteps++;
		tempResultArray[1] = Math.sqrt(tempTotalDeviationRMSE / tempTestingObjectCount);
		runSteps++;
	...
	public static void main(String[] args) {
		try {
			...
			EMTMJ tempRecommender = new EMTMJ(10000, 2000); // douban:2965,
			System.out.println("CPCC");
			for (int tempK = 5; tempK < 105; tempK += 5) {
				System.out.println("tempK: " + tempK);
				experimentKNNMAE(tempRecommender, tempK, CPCC);
			} // of for tempK
	 ...
	 ************************* 
	 */
	public static void experimentKNNMAE(EMTMJ paraRecommender, int parak, int paraMeasure) {
		double[] tempResultArray = new double[2];
		try {
			...
			System.out.println("The MAE=" + tempResultArray[0] + "; The RMSE=" + tempResultArray[1]);
			// }// Of for tempK
		} catch (Exception ee) {
			ee.getStackTrace();
			System.out.println(ee);
