...
//import ARSource.SimpleTool;

public class M100KTMJ {
	 ...
	 ************************* 
	 */
	public M100KTMJ(int paraUsers, int paraItems) throws Exception {
		...
		String tempReaderfile = "data/movielens/100k/newRatings2.txt"; // newRatings2
		...
		for (int i = 0; i < itemDegree.length; i++) {
			...
			for (int j = 0; j < itemDegree[i]; j++) {
				...
				standarddeviation[i] += (itemRatingInformation[i][j] - itemAveRating[i])
						* (itemRatingInformation[i][j] - itemAveRating[i]);
			} // Of for j

		} // Of for i
			// System.out.println(Arrays.deepToString(itemRatingInformation));
	 ...
	 ************************* 
	 */
	public void leaveOneOut(int paraItemIndex, int paraItemPosition, int paraUserIndex, 
			double paraValue) {
	...
	public void InitializeSpaceBigToSmall(int[] paraNeighborsIndice, double[] paraNeighborsDistance,
		...
		paraNeighborsDistance[0] = 10000000;
		paraCheckIndice[0] = -1;
	...
	public double PIP(int paraRow1, int paraRow2) throws Exception {
		...
		while (i < itemRatingIndices[paraRow1].length && j < itemRatingIndices[paraRow2].length) {
			if (itemRatingIndices[paraRow1][i] < itemRatingIndices[paraRow2][j]
				...
				double tempImpact = 0;
				if (agreeMent) {
					tempImpact = (Math.abs(itemRatingInformation[paraRow1][i] - 3) + 1)
							* (Math.abs(itemRatingInformation[paraRow2][j] - 3) + 1);
				} else {
					tempImpact = 1 / ((Math.abs(itemRatingInformation[paraRow1][i] - 3) + 1)
							* (Math.abs(itemRatingInformation[paraRow2][j] - 3) + 1));
				}
	...
	public double NHSM(int paraRow1, int paraRow2) throws Exception {
		...
		while (i < itemRatingIndices[paraRow1].length && j < itemRatingIndices[paraRow2].length) {
			if (itemRatingIndices[paraRow1][i] < itemRatingIndices[paraRow2][j]
				...
				Significance = 1 / (1 + Math.exp(-Math.abs(itemRatingInformation[paraRow1][i] - 3)
	...
	public double BC(int paraRow1, int paraRow2) throws Exception {
		...
		for (int x = 0; x < 5; x++) {
			BSum += Math.sqrt((everyItemClassfication[0][x] / everyItemClassfication[0][5])
	...
	public static void main(String[] args) {
		try {
			M100KTMJ tempRecommender = new M100KTMJ(943, 1682);
			// ML100KJCT tempRecommender = new ML100KJCT(4, 4);
			for (int tempK = 5; tempK < 105; tempK += 5) {
				System.out.print("tempK: " + tempK);
				experimentKNNMAE(tempRecommender, tempK, TMJ);
			} // of for tempK
	 ...
	 ************************* 
	 */
	public static void experimentKNNMAE(M100KTMJ paraRecommender, int parak, int paraMeasure) {
		double[] tempResultArray = new double[2];
		try {
