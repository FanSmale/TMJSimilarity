...
public class FTTMJ {
	 ...
	 ************************** 
	 */
	public FTTMJ(int paraUsers, int paraItems) throws Exception {
		...
		String tempReaderfile = "data/filmtrust/ratings.txt";
		File tempFile = null;
		...
		double[][] tempItemRatings = new double[itemNumber][userNumber];
		itemDegree = new int[itemNumber];
		int[][] tempUserIndices = new int[userNumber][itemNumber];
		double[][] tempUserRatings = new double[userNumber][itemNumber];
	...
	public double[] computeOneDeviationKNNMAE(int paraItemIndex, int[] paraUserIndexArray, double[] paraRatingArray,
		...
		for (int i = 0; i < paraUserIndexArray.length; i++) {
			...
			// System.out.println("The user position " + i);
			if (paraMeasure == EUCLIDEAN) {
				...
				try {
					tempPrediction = predictKNN(paraItemIndex, i, paraUserIndexArray, paraRatingArray, paraK,
							paraMeasure);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
			...
			runSteps++;
			tempResult[1] += (tempPrediction - leaveOutValue) * (tempPrediction - leaveOutValue);
			runSteps++;
			// Step 4. Restore.
			leaveOneOutRestore();
			runSteps++;
	...
	public double predictKNNDistance(int paraPredictItemArrayIndex, int paraUserPosition, int[] paraUserIndexArray,
		...
		InitializeSpaceSmallToBig(tempNeighborIndices, tempNeighborDistances, tempcheckIndices); // ��С����
	...
	private double predictKNN(int paraPredictItemArrayIndex, int paraUserPosition, int[] paraUserIndexArray,
		...
		double[] tempNeighborDistances = new double[paraK + 2];
		int[] tempcheckIndices = new int[paraK + 2];
		InitializeSpaceBigToSmall(tempNeighborIndices, tempNeighborDistances, tempcheckIndices); // �Ӵ�С
		...
		for (int i = 0; i < itemNumber; i++) {
			...
			// System.out.println(Arrays.toString(tempNeighborDistances));
			// Step 3. Predict.
		double tempPrediction = 0;
		double tempSumSimilarity = 0;
		for (int i = 1; i <= paraK; i++) {
			if (tempNeighborIndices[i] != -1) {
				neighborScoreTotal += (itemRatingInformation[tempNeighborIndices[i]][tempcheckIndices[i]]
						- itemAveRating[tempNeighborIndices[i]]) * tempNeighborDistances[i];
				tempSumSimilarity += tempNeighborDistances[i];
				// System.out.println("tempSumSimilarity: "
				// +tempSumSimilarity);
			} // of if
		} // of for i
	...
	public double PIP(int paraRow1, int paraRow2) throws Exception {
		...
		while (i < itemRatingIndices[paraRow1].length && j < itemRatingIndices[paraRow2].length) {
			if (itemRatingIndices[paraRow1][i] < itemRatingIndices[paraRow2][j]
				...
				if ((itemRatingInformation[paraRow1][i] > 2.5 && itemRatingInformation[paraRow2][j] < 2.5)
						|| (itemRatingInformation[paraRow1][i] < 2.5 && itemRatingInformation[paraRow2][j] > 2.5)) {
					agreeMent = false;
				}
				...
				double tempImpact = 0;
				if (agreeMent) {
					tempImpact = (Math.abs(itemRatingInformation[paraRow1][i] - 2.5) + 1)
							* (Math.abs(itemRatingInformation[paraRow2][j] - 2.5) + 1);
				} else {
					tempImpact = 1 / ((Math.abs(itemRatingInformation[paraRow1][i] - 2.5) + 1)
							* (Math.abs(itemRatingInformation[paraRow2][j] - 2.5) + 1));
				}
	...
	public double NHSM(int paraRow1, int paraRow2) throws Exception {
		...
		while (i < itemRatingIndices[paraRow1].length && j < itemRatingIndices[paraRow2].length) {
			if (itemRatingIndices[paraRow1][i] < itemRatingIndices[paraRow2][j]
				...
				Significance = 1 / (1 + Math.exp(-Math.abs(itemRatingInformation[paraRow1][i] - 2.5)
						* Math.abs(itemRatingInformation[paraRow2][j] - 2.5)));
				// Popularity
	...
	public double BC(int paraRow1, int paraRow2) throws Exception {
		...
		everyItemClassfication = new double[2][9];
		...
		for (int j = 0; j < itemRatingInformation[paraRow2].length; j++) {
			...
			everyItemClassfication[1][8]++;
		} // of for j

		double BSum = 0;

		for (int x = 0; x < 5; x++) {
			BSum += Math.sqrt((everyItemClassfication[0][x] / everyItemClassfication[0][8])
					* (everyItemClassfication[1][x] / everyItemClassfication[1][8]));
	...
	public double CPCC(int paraRow1, int paraRow2) throws Exception {
		...
		while (i < itemRatingIndices[paraRow1].length && j < itemRatingIndices[paraRow2].length) {
			if (itemRatingIndices[paraRow1][i] < itemRatingIndices[paraRow2][j]
				...
				tempFirstUserRating = itemRatingInformation[paraRow1][i] - 2.5;
				tempSecondUserRating = itemRatingInformation[paraRow2][j] - 2.5;
				tempXYSum += tempFirstUserRating * tempSecondUserRating;
				tempXSum += tempFirstUserRating * tempFirstUserRating;
				tempYSum += tempSecondUserRating * tempSecondUserRating;
				i++;
	...
	public static void main(String[] args) {
		try {
			FTTMJ tempRecommender = new FTTMJ(1508, 2071); // douban:2965,
			for (int tempK = 5; tempK < 75; tempK += 5) {
	 ...
	 ************************* 
	 */
	public static void experimentKNNMAE(FTTMJ paraRecommender, int parak, int paraMeasure) {
		double[] tempResultArray = new double[2];
		try {
